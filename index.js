...
function makeQuiz(questions, quizContainer, resultsContainer, submitButton){
    ...
    function showQuestions(questions, quizContainer){
        ...
        for(let i=0;i<questions.length;i++) {
            ...
            for(let letter in questions[i].options){
                answers.push(
                    '<label>'
                    + '<input class= "radios" type="radio" name="question'+i+'" value="'+letter+'" style="margin-right: 15px">'
                    + letter + ': '
            ...
            output.push(
                '<div class="question">' + questions[i].prompt + '</div>'
    ...
    function showResults(questions, quizContainer, resultsContainer){
        ...
        var modeMap = {};
        var maxEl = (answerContainers[0].querySelector('input[name=question'+i+']:checked')||{}).value;
        var maxCount = 1;
        ...
        for(var i=0; i<questions.length; i++){
            ...
            if(modeMap[userAnswer] == null)
                modeMap[userAnswer] = 1;
            else
                modeMap[userAnswer]++;
            if(modeMap[userAnswer] > maxCount)
            {
                maxEl = userAnswer;
                maxCount = modeMap[userAnswer];
            }

        }

        if (answeredAll){
            resultsContainer.textContent = data.results[maxEl.charCodeAt(0) - 65];
            ...
            retakeContainer.innerHTML = '<input class="button" type="button" value="Retake Quiz" />';
            retakeContainer.onclick = function(){
                resultsContainer.style.display = "none";
                submitButton.style.display = "block"
                retakeContainer.innerHTML = '';
                resultsContainer.innerHTML = '';
                makeQuiz(data.questions, quizContainer, resultsContainer, submitButton);
            }
        } else{
...
var retakeContainer = document.getElementById('retake');
